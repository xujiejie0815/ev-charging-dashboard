...
# カスケードフィルター
cats = ["すべて"] + sorted(df_base["カテゴリー"].dropna().unique().tolist())
sel_cat = st.sidebar.selectbox("カテゴリー", cats)

df_f1 = df_base if sel_cat == "すべて" else df_base[df_base["カテゴリー"] == sel_cat]

brands = ["すべて"] + sorted([b for b in df_f1["ブランド"].dropna().unique() if b])
sel_brand = st.sidebar.selectbox("ブランド", brands)

df_f2 = df_f1 if sel_brand == "すべて" else df_f1[df_f1["ブランド"] == sel_brand]

models = ["すべて"] + sorted(df_f2["モデル"].dropna().unique().tolist())
sel_model = st.sidebar.selectbox("モデル", models)

df_f3 = df_f2 if sel_model == "すべて" else df_f2[df_f2["モデル"] == sel_model]
...
# ──────────────────────────────────────────
with tab1:
    fac_names = sorted(df_filtered["施設名"].dropna().unique().tolist())
    if not fac_names:
        st.warning("条件に合う施設がありません。")
    else:
        sel_fac = st.selectbox("施設名", fac_names, key="fac_select")
        df_fac = df_filtered[df_filtered["施設名"] == sel_fac].sort_values("date")
